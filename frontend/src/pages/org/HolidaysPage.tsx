...
export function HolidaysPage() {
  ...
  const updateMutation = useUpdateHolidayCalendar(editingId ?? '')
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Define and publish annual holiday calendars, then assign them to office locations."
        actions={
      ...
      <AppDialog
        ...
        title={editingId ? 'Edit holiday calendar' : 'Create holiday calendar'}
        description="Holiday calendars are date-based and managed year by year, while leave cycles remain separately configurable."
        contentClassName="sm:w-[min(92vw,56rem)]"
        footer={
          <div className="flex flex-wrap justify-end gap-3">
            <button type="button" className="btn-secondary" onClick={resetForm}>
              Cancel
            </button>
            <button type="submit" form="holiday-calendar-form" className="btn-primary" disabled={createMutation.isPending || updateMutation.isPending}>
              {editingId ? 'Save changes' : 'Save holiday calendar'}
            </button>
          </div>
        }
      >
        <form id="holiday-calendar-form" onSubmit={handleSubmit} className="grid gap-4">
          <div>
