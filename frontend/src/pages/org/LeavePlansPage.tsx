...
export function LeavePlansPage() {
  ...
  const totalLeaveTypes = (plans ?? []).reduce((sum, plan) => sum + plan.leave_types.length, 0)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button type="button" className="btn-primary" onClick={() => navigate('/org/leave-plans/new')}>
            Build new plan
          </button>
        }
      />
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <div className="surface-card rounded-[28px] p-5">
          ...
          <p className="mt-3 text-3xl font-semibold text-[hsl(var(--foreground-strong))]">{plans?.length ?? 0}</p>
        </div>
        <div className="surface-card rounded-[28px] p-5">
            ...
            <div key={plan.id} className="surface-muted rounded-[24px] p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button type="button" className="btn-secondary" onClick={() => navigate(`/org/leave-plans/${plan.id}`)}>
                    Open builder
                  </button>
                </div>
              </div>
