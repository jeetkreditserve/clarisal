      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <SectionCard title="Education history" description="Existing records associated with your employee profile.">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {data.map((record) => (
                <div key={record.id} className="flex flex-col gap-4 rounded-[24px] border border-slate-200 bg-slate-50 p-5 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      <GraduationCap className="h-4 w-4 text-cyan-700" />
                      <p className="font-semibold text-slate-950">{record.degree}</p>
                    </div>
                    <p className="mt-2 text-sm text-slate-600">{record.institution}</p>
                    <p className="mt-1 text-sm text-slate-500">
                      {record.field_of_study || 'General'} • {record.start_year || 'N/A'} - {record.is_current ? 'Present' : record.end_year || 'N/A'}
                    </p>
                    <p className="mt-2 text-xs uppercase tracking-[0.16em] text-slate-500">Updated {formatDate(record.updated_at)}</p>
                  </div>
