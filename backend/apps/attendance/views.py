...
class OrgAttendanceShiftAssignmentListView(APIView):
    ...
    def get(self, request):
        organisation = _get_admin_organisation(request)
        assignments = organisation.shift_assignments.select_related('employee__user', 'shift').order_by('-start_date', 'employee__employee_code')
...
class OrgAttendanceDayDetailView(APIView):
    ...
    def patch(self, request, pk):
        organisation = _get_admin_organisation(request)
        attendance_day = get_object_or_404(AttendanceDay.objects.select_related('employee'), organisation=organisation, id=pk)
        ...
        check_in_at = (
            ...
            if check_in is None
            else timezone.make_aware(datetime.combine(attendance_day.attendance_date, check_in), timezone.get_default_timezone())
        )
        check_out_at = (
            attendance_day.check_out_at
            if check_out is None
            else timezone.make_aware(datetime.combine(attendance_day.attendance_date, check_out), timezone.get_default_timezone())
        )
        try:
...
class OrgAttendanceRegularizationListView(APIView):
    ...
    def get(self, request):
        organisation = _get_admin_organisation(request)
        queryset = list_attendance_regularizations_for_org(organisation, status_value=request.query_params.get('status', ''))
    ...
    def post(self, request):
        ...
        requested_check_in_at = (
            ...
            if serializer.validated_data.get('requested_check_in') is None
                ...
                datetime.combine(serializer.validated_data['attendance_date'], serializer.validated_data['requested_check_in']),
        ...
        requested_check_out_at = (
            ...
            if serializer.validated_data.get('requested_check_out') is None
                ...
                datetime.combine(serializer.validated_data['attendance_date'], serializer.validated_data['requested_check_out']),
                timezone.get_default_timezone(),
            )
        )
        try:
...
class MyAttendanceRegularizationDetailView(APIView):
    ...
    def patch(self, request, pk):
        employee = _get_self_employee(request)
        regularization = get_object_or_404(AttendanceRegularizationRequest, organisation=employee.organisation, employee=employee, id=pk)
        if regularization.status != 'PENDING':
            return Response({'error': 'Only pending regularization requests can be edited.'}, status=status.HTTP_400_BAD_REQUEST)
