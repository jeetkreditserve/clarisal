...
def _parse_datetime_value(value, *, label, policy=None):
    ...
    if isinstance(value, datetime):
        return (
            ...
            else timezone.make_aware(value, _policy_timezone(policy) if policy else timezone.get_default_timezone())
        )
    text = _strip_string(value)
    for fmt in (
        "%Y-%m-%dT%H:%M:%S",
    ...
    try:
        ...
        return (
            ...
            else timezone.make_aware(parsed, _policy_timezone(policy) if policy else timezone.get_default_timezone())
        )
...
def upsert_attendance_policy(organisation, *, actor=None, policy=None, **fields):
    ...
    if target_is_default:
        AttendancePolicy.objects.filter(organisation=organisation).exclude(id=getattr(policy, "id", None)).update(
            is_default=False
        )
    if policy is None:
        policy = AttendancePolicy.objects.create(
...
def create_source_config(organisation, *, name, kind, actor=None, configuration=None, is_active=True):
    ...
    log_audit_event(
        actor, "attendance.source.created", organisation=organisation, target=source, payload={"kind": kind}
    )
...
def update_source_config(source, *, actor=None, name=None, is_active=None, configuration=None, rotate_api_key=False):
    ...
    if changed:
        ...
        log_audit_event(
            ...
            target=source,
            payload={"rotated_api_key": rotate_api_key},
        )
    return source, raw_api_key
...
def _get_effective_shift(employee, attendance_date):
    rotation_assignment = (
        employee.shift_rotation_assignments.filter(
            organisation=employee.organisation,
            is_active=True,
            start_date__lte=attendance_date,
        )
        .filter(Q(end_date__isnull=True) | Q(end_date__gte=attendance_date))
    ...
    direct_assignment = (
        employee.shift_assignments.filter(
            organisation=employee.organisation,
            is_active=True,
            start_date__lte=attendance_date,
        )
...
def _get_on_duty_fraction(employee, attendance_date, half_day_minutes):
    ...
    for request in OnDutyRequest.objects.filter(
        ...
        if (
            request.duration_type == OnDutyDurationType.TIME_RANGE
            and request.start_date == request.end_date == attendance_date
        ):
...
def _pick_interval_from_punches(punches):
    ...
    check_out_candidates = [
        item.punch_at for item in punches if item.action_type == AttendancePunchActionType.CHECK_OUT
    ]
...
def _summarize_day(employee, attendance_date):
    ...
    if leave_fraction >= FULL:
        ...
        needs_regularization = False
    elif on_duty_fraction == HALF and status in {
        ...
        paid_fraction = FULL
        needs_regularization = False
    elif wfh_fraction == HALF and status in {
        AttendanceDayStatus.ABSENT,
        AttendanceDayStatus.HALF_DAY,
        AttendanceDayStatus.INCOMPLETE,
    }:
...
def _month_window(month, *, policy=None):
    ...
    next_month = date(
        start_date.year + (1 if start_date.month == 12 else 0), 1 if start_date.month == 12 else start_date.month + 1, 1
    )
...
def get_employee_attendance_calendar(employee, *, month=None):
    ...
    return {
        "month": month
        or _month_window(None, policy=get_default_attendance_policy(employee.organisation))[0].strftime("%Y-%m"),
        "days": [
            {
...
def _validate_geo_fence(
    ...
    *,
    ...
    policies = GeoFencePolicy.objects.filter(
...
def validate_mobile_punch_location(
    organisation_id: str,
    latitude: float | None,
    longitude: float | None,
    *,
...
def calculate_attendance_day_status(
    ...
    ins = sorted([punch["punch_time"] for punch in punches if punch.get("direction", "IN") == "IN"])
    outs = sorted([punch["punch_time"] for punch in punches if punch.get("direction") == "OUT"])
...
def record_employee_punch(
    employee, *, action_type, actor=None, remote_ip="", latitude=None, longitude=None, source=AttendancePunchSource.WEB
):
    policy = get_default_attendance_policy(employee.organisation)
    ...
    if (
        ...
        and attendance_day.check_in_at
        and not attendance_day.check_out_at
    ):
    ...
    for index, payload in enumerate(punches, start=1):
        ...
        try:
            ...
            punch = AttendancePunch.objects.create(
                ...
                if source_config.kind == AttendanceSourceConfigKind.API
                else AttendancePunchSource.MANUAL,
                punch_at=punch_at,
...
def _publish_punch_event(
    ...
    try:
        from apps.biometrics.events import publish_punch_event
        ...
        logger.exception("Failed to publish punch event")
...
def create_regularization_request(
    employee, *, attendance_date, requested_check_in_at=None, requested_check_out_at=None, reason, actor=None
):
    ...
    log_audit_event(
        actor or employee.user,
        "attendance.regularization.created",
        organisation=employee.organisation,
...
def withdraw_regularization_request(regularization, *, actor=None):
    ...
    log_audit_event(
        actor, "attendance.regularization.withdrawn", organisation=regularization.organisation, target=regularization
    )
    return regularization
...
def apply_regularization_status_change(regularization, new_status, rejection_reason=""):
    ...
    if new_status == AttendanceRegularizationStatus.APPROVED:
        ...
        check_in_at = (
            regularization.requested_check_in_at or existing_day.check_in_at or regularization.requested_check_out_at
        )
...
def get_org_attendance_dashboard(organisation, *, target_date=None):
    ...
    employees = Employee.objects.filter(organisation=organisation, status=EmployeeStatus.ACTIVE).select_related(
...
def get_org_attendance_report(organisation, *, month=None):
    ...
    employees = list(
        Employee.objects.filter(organisation=organisation, status=EmployeeStatus.ACTIVE).select_related("user")
    )
    summary = {
    ...
    while current <= period_end:
        for employee in employees:
            ...
            if (
                day.status in {AttendanceDayStatus.ABSENT, AttendanceDayStatus.INCOMPLETE}
                or day.is_late
                or day.overtime_minutes
            ):
...
def list_org_attendance_days(organisation, *, target_date=None, employee_id=None, status_value=""):
    ...
    day_summaries = [
        recalculate_attendance_day(employee, attendance_date)
        for employee in employees.select_related("user", "office_location")
    ]
    if status_value:
...
def get_payroll_attendance_summary(employee, *, period_start, period_end):
    ...
    return {
        "paid_days": int(total_paid_fraction)
        if total_paid_fraction == total_paid_fraction.to_integral()
        else float(total_paid_fraction),
    ...
    with transaction.atomic():
        ...
        for row in rows:
            ...
            try:
                ...
                check_in_at = _make_aware_datetime(
                    attendance_date, _parse_time_value(row.get("check_in"), label="Check in"), policy
                )
                check_out_at = _make_aware_datetime(
                    attendance_date, _parse_time_value(row.get("check_out"), label="Check out"), policy
                )
        ...
        log_audit_event(
            uploaded_by,
            "attendance.import.posted",
            organisation=organisation,
            target=job,
            payload={"mode": job.mode, "posted_rows": posted_rows},
        )
        return job
    ...
    with transaction.atomic():
        ...
        for row in rows:
            ...
            try:
                normalized_code, employee = _resolve_employee(organisation, employee_code)
                attendance_date = _parse_date(row.get("date"))
                punch_at = _make_aware_datetime(
                    attendance_date, _parse_time_value(row.get("punch_time"), label="Punch time"), policy
                )
                grouped_punches[(employee.id, attendance_date)].append(
                    (normalized_code, employee, punch_at, row["row_number"])
                )
            except ValueError as exc:
                AttendanceImportRow.objects.create(
                    job=job,
        ...
        generated_rows = 0
        for (_employee_id, attendance_date), punch_rows in sorted(
            grouped_punches.items(), key=lambda item: (item[0][1], item[0][0])
        ):
            ...
            error_message = (
                ""
                if check_out_at
                else "Only one punch was found for this employee on this date. Add a checkout time before re-uploading."
            )
            AttendanceImportRow.objects.create(
                job=job,
        ...
        log_audit_event(
            uploaded_by,
            "attendance.import.normalized",
            organisation=organisation,
            target=job,
            payload={"mode": job.mode, "generated_rows": generated_rows},
        )
        return job
